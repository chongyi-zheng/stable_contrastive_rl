...
from rlkit.torch.sac.contrastive_rl_trainer import ContrastiveRLTrainer
...
# from rlkit.experimental.chongyiz.networks.encoding_networks import (
#     ContrastiveCNNObsEncoder,
#     ContrastiveResCNNObsEncoder
# )

from rlkit.experimental.chongyiz.learning.contextual_replay_buffer import ContextualRelabelingReplayBuffer
from rlkit.experimental.chongyiz.learning.online_offline_split_replay_buffer import OnlineOfflineSplitReplayBuffer
from rlkit.experimental.chongyiz.networks.contrastive_networks import (
    ContrastiveQf,
    ContrastiveVf,
)
...
def contrastive_rl_experiment(  # NOQA
        max_path_length,
        qf_kwargs,
        vf_kwargs,
        # obs_encoder_kwargs,
    ...
    # (chongyiz): remove planer arguments
    ...
    # TODO (chongyiz): remove obs_encoder
    ...
    # TODO (chongyiz): replace q network output with product between sa_repr and g_repr
    def create_qf():
        ...
        if use_vqvae:
            qf_kwargs['state_dim'] = obs_dim
            qf_kwargs['sa_dim'] = obs_dim + action_dim
            qf_kwargs['g_dim'] = goal_dim
        else:
        ...
        # qf_kwargs['init_w'] = 1e-12
        qf_kwargs['hidden_init'] = partial(
            nn.init.xavier_uniform_,
            gain=nn.init.calculate_gain('relu'))
        # qf_kwargs['layer_norm'] = True
    ...
    def create_vf():
        ...
        if use_vqvae:
            vf_kwargs['state_dim'] = obs_dim
        else:
            assert network_type == "contrastive_cnn"

            vf_kwargs['use_image_obs'] = True
            vf_kwargs['imsize'] = imsize

        vf_kwargs['hidden_init'] = partial(
            nn.init.xavier_uniform_,
            gain=nn.init.calculate_gain('relu'))
        # vf_kwargs['layer_norm'] = True
    ...
    if policy_class in [GaussianCNNPolicy]:
    ...
    elif policy_class in [GaussianTwoChannelCNNPolicy]:
        assert policy_kwargs['output_activation'] is None
        policy_kwargs['input_width'] = imsize
        policy_kwargs['input_height'] = imsize
        ...
        policy_kwargs['conv_normalization_type'] = 'layer' if policy_layer_norm else 'none'
        policy_kwargs['fc_normalization_type'] = 'layer' if policy_layer_norm else 'none'
    elif policy_class in [GoalConditionedGaussianFixedReprPolicy]:
        policy_kwargs['input_width'] = imsize
        policy_kwargs['input_height'] = imsize
        policy_kwargs['input_channels'] = 6
        policy_kwargs['fc_normalization_type'] = 'layer' if policy_layer_norm else 'none'

    # DELETEME (chongyiz): we don't use encoder in policy
    ...
    # TODO (chongyiz): refactoring the following code
    if use_vqvae:
        behavioral_cloning_policy = policy_class(
            obs_dim=obs_dim,
            action_dim=action_dim,
            **policy_kwargs,
        )
    elif use_image:
        # TODO (chongyiz): implement dropout for GaussianCNNPolicy
        behavioral_cloning_policy = None
    ...
    # TODO (chongyiz): implement goal relabeling somewhere here
    ...
    # TODO (chongyiz): do we need to make evaluation policy deterministic here?
    ...
    if trainer_type == 'iql':
        ...
        trainer = ContrastiveRLTrainer(
            env=eval_env,
            policy=policy,
            behavioral_cloning_policy=behavioral_cloning_policy,
            qf=qf,
            # qf2=qf2,
            target_qf=target_qf,
