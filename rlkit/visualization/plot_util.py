...
def extract_comparison(exp, method_name, metric_key, smooth=10):
    ...
    for i in range(length):
        ...
        if i % smooth == 0:
            ...
            value += float(exp[metric_key][i] / smooth)

        if (i + 1) % smooth == 0:
            data_i = [method_name, epoch, value]
            data.append(data_i)

    return data


def load_comparisons(exp_dir,
                     method_name_to_path,
                     task_path,
                     metric_key):
    data = []

    for method_name, method_path in method_name_to_path.items():
        if task_path is None:
            dirs = glob.glob(os.path.join(exp_dir, method_path))
        else:
            dirs = glob.glob(os.path.join(exp_dir, task_path, method_path))

        experiments = load_exps(dirs, suppress_output=True)

        for exp in experiments:
            data += extract_comparison(
                exp['progress'],
