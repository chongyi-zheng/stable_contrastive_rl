...
from rlkit.torch.sac.policies import GaussianPolicy  # NOQA
from rlkit.torch.sac.policies import MakeDeterministic  # NOQA
from rlkit.torch.sac.iql_bellman_trainer import IQLVIBTrainer  # NOQA
from rlkit.torch.sac.iql_bellman_trainer_v2 import IQLVIBTrainerV2  # NOQA
...
from rlkit.experimental.kuanfang.networks.encoding_networks import DeterministicObsEncoder  # NOQA
...
# from rlkit.experimental.kuanfang.networks.value_networks import L2VNetwork, L2QNetwork  # NOQA
from rlkit.experimental.kuanfang.networks.value_networks import *  # NOQA
...
image_to_latent_map = {
    'image_observation': 'latent_observation',
    'image_desired_goal': 'latent_desired_goal',
}
...
def ptp_experiment(  # NOQA
    ...
    # else:
    #     raise ValueError('Unrecognized observation_key: %s' %
    #                      (observation_key))

    # if observation_key == 'image_observation':
    #     assert initial_state_key == 'initial_image_observation'
    #     assert goal_key == 'image_desired_goal'
    # elif observation_key == 'latent_observation':
    #     assert initial_state_key == 'initial_latent_state'
    ...
    # vqvae = model['vqvae']
    # affordance = model['affordance']  # TODO
    ...
    #         )
    #
    #     return contextual_env, latent_goal_distribution, reward_fn
    #
    ...
    policy = None
    obs_encoder = None
    obs_encoding_dim = obs_dim

    # if trainer_type == 'v1':
    #     obs_encoder_class = VariationalObsEncoder
    # elif trainer_type == 'v2':
    ...
    if network_type == 'mlp':
        ...
        qf_class = BilinearQNetwork
        vf_class = BilinearVNetwork
    elif network_type == 'my':
        qf_class = MyMlpQNetwork
        vf_class = MyMlpVNetwork
    else:
        ValueError
    ...
    def create_qf():
        ...
        if value_bias is not None:
            qf_kwargs['last_b_init_value'] = value_bias
        return qf_class(
            **qf_kwargs
        )
    qf1 = create_qf()
    ...
    def create_vf():
        ...
        if value_bias is not None:
            vf_kwargs['last_b_init_value'] = value_bias
        return vf_class(
            **vf_kwargs
        )
    vf = create_vf()
    ...
    #
    #     # print('-----------')
    #     # for cont_key, cont_value in new_contexts.items():
    #     #     print(cont_key, cont_value.shape)
    #
    #     # if len(new_contexts.keys()) > 1:
    #     #     context = np.concatenate(tuple(new_contexts.values()), axis=1)
    #     # else:
    #     #     context = batch[goal_key]
    #
    ...
    #         image_to_latent_map=image_to_latent_map,
    #         **replay_buffer_kwargs
    #     )
    # else:
    ...
    #     expl_save_video_kwargs['image_goal_key'] = image_goal_key
    #
    #     expl_video_func = RIGVideoSaveFunction(
    #         vqvae,
    #         expl_path_collector,
    #         'train',
    #         # decode_image_goal_key='image_decoded_goal',
    ...
    #     eval_save_video_kwargs['image_goal_key'] = image_goal_key
    #
    ...
    #     path_loader_kwargs['model'] = vqvae
    #
    #     if add_env_demos:
    ...
    #     path_loader = EncoderDictToMDPPathLoader(
    #         trainer,
    #         replay_buffer=replay_buffer,
    #         demo_train_buffer=demo_train_buffer,
    #         # demo_test_buffer=demo_test_buffer,
    #         demo_test_buffer=eval_replay_buffer,  # TODO
    #         reward_fn=eval_reward,
    ...
    #     pt_path = osp.join(logger.get_snapshot_dir(),     'pretrain_algorithm.pt')  # NOQA
