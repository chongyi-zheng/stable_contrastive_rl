...
class Encoder(nn.Module):
    ...
    def forward(self, inputs):
        x = self._conv_1(inputs)  # (B, 64, 24, 24)
        x = F.relu(x, inplace=True)

        x = self._conv_2(x)  # (B, 128, 12, 12)
        x = F.relu(x, inplace=True)

        x = self._conv_3(x)  # (B, 128, 12, 12)
        # x = F.relu(x, inplace=True)

        # x = self._residual_stack(x)  # (B, 128, 12, 12)
...
class Decoder(nn.Module):
    ...
    def forward(self, inputs):
        x = self._conv_1(inputs)  # (B, 128, 12, 12)
        x = F.relu(x, inplace=True)

        # x = self._residual_stack(x)  # (B, 128, 12, 12)

        x = self._conv_trans_1(x)  # (B, 64, 24, 24)
        x = F.relu(x, inplace=True)

        x = self._conv_trans_2(x)  # (B, 3, 48, 48)
