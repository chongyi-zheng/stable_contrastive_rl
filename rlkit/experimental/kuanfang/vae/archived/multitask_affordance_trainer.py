...
class AffordanceTrainer(LossFunction):
    ...
    def __init__(
        ...
        self.affordance_params = []
        for network in self.affordance.networks:
            self.affordance_params.extend(list(network.parameters()))
    ...
    def train_epoch(self, epoch, dataloader, batches=100):
        ...
        for b in range(batches):
            # TODO
            k = np.random.randint(self.affordance.num_tasks)
            dataloader.dataset.set_skill_id(k)
    ...
    def train_batch(self, epoch, batch, should_log, should_train_classifier):
        ...
        self.tf_logger.log_histogram(
            '%s/skill_id' % (prefix),
            ptu.get_numpy(skill_id),
            epoch)

        skill_id = ptu.get_numpy(skill_id)
        assert np.sum(np.abs(skill_id - skill_id[0:1])) == 0
        skill_id = int(skill_id[0])
        ...
        aff_loss, aff_extra = self.compute_affordance_loss(
            s, h, skill_id, epoch, prefix, should_log)
    ...
    def compute_affordance_loss(
            self, s, h, skill_id, epoch, prefix, should_log):
    ...
    def _compute_affordance_loss(
            self, h0, h1, skill_id, weights, epoch, prefix, should_log):
        ...
        (z_mu, z_logvar), z, zes_pred = self.affordance(
            zqs_t1, cond=zqs_t0, skill_id=skill_id)

        # TODO
        # (z_mu, z_logvar), z, zes_pred = self.affordance.networks[0](
        #     zqs_t1, cond=zqs_t0)
    ...
    def _plot_images(self, plot_data, epoch, prefix):
        ...
        # Sample goals.
        for k in range(self.affordance.num_tasks):
            s0 = plot_data['s'][:, 0]
            h0 = plot_data['h'][:, 0]
            ...
            for t in range(num_steps - 1):
                h_pred, goal_pred = self._sample_goals(h_last, k)
            ...
            image = torch.cat(goal_preds, dim=-2) + 0.5
            image = image.permute(0, 2, 3, 1).contiguous()
            image = ptu.get_numpy(image)
            self.tf_logger.log_images(
                '%s_sampled_goals_%d' % (prefix, k),
                image,
                epoch)

    def _sample_goals(self, h0, skill_id):
        ...
        z = ptu.from_numpy(z)
        h1_pred = self.affordance.decode(z, cond=h0, skill_id=skill_id)
